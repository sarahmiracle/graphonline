    ...
    var child = exec(execstr,{cwd:configf.curdir + work_dir.substr(1)},
        function (error, stdout, stderr) {
            for(i=0;i<files.length;i++) {
                (function(filename) {
                    setTimeout(function () {
                        fs.removeSync(work_dir + filename);
                    }, 60 * 1000);
                })(files[i]);
            }
        });
...
var start_program = function()
{
    if(process.argv.length!=7 && process.argv.length!=4)
    {
        ...
        console.log('usage: node find_errors.js left right start_part parts group_size');
        console.log('OR node find_errors.js filename group_size');
        return;
    }
